...
<body>
    ...
    <input type="radio" id="preuvelivraison" name="type" value="preuve de livraison" onchange="handleTypeChange(this)">        <label for="preuvelivraison">Preuves de livraison</label>
    </div>
    <div class="radio-group">
    <input type="radio" id="fiherenseignement" name="type" value="fiche de renseignements" onchange="handleTypeChange(this)">
        <label for="fiherenseignement">Fiche de renseignement</label>
    </div>
    <div class="radio-group">
    <input type="radio" id="fichexpedition" name="type" value="fiche expedition" onchange="handleTypeChange(this)">
        <label for="fichexpedition">Fiche d'expédition</label>
    </div>
...
<div class="form-group" id="dateExpirationField" style="display: none">
    <label for="dateExpiration">Date fin :</label>
    ...
    <script type="text/javascript">
        ...
        function handleTypeChange(radioElement) {
    const dateExpirationField = document.getElementById("dateExpirationField");
    
    if (radioElement.value === "fiche expedition") {
...
let expirationDate;
    if (type === "fiche expedition") {
        expirationDate = document.getElementById("dateExpiration").value;
        if (!expirationDate) {
            alert("Please enter an expiration date for the Fiche d'expédition.");
            return;
        }
    }
if (!type) {
    ...
    if (DWObject && DWObject.HowManyImagesInBuffer > 0) {
        ...
        DWObject.SaveAllAsPDF(title + '.pdf', function() {
            ...
            DWObject.ConvertToBlob(
                ...
                function (blob) {
                    uploadPDFToServer(title, blob, expirationDate);
                },
                OnFailure
            );
        }, OnFailure);
...
function uploadPDFToServer(title, pdfBlob, expirationDate) {
